...
export function drawHud() {
  ...
  ctx.fillText(String(Math.max(0, Math.round(game.score + game.health * 100))), 18, 14);
  ctx.font = "600 10px system-ui, sans-serif";
  ctx.fillStyle = "rgba(233, 246, 241, 0.6)";
  ctx.fillText(`SCORE   ◆ ${game.orbCount}`, 19, 43);

  // Countdown (top-right)
  const remain = Math.max(0, Math.ceil(RUN_SECONDS - game.elapsed));
  const time = `${Math.floor(remain / 60)}:${String(remain % 60).padStart(2, "0")}`;
  ctx.textAlign = "right";
  ctx.fillStyle = remain <= 10 ? "#ff7a45" : "#e9f6f1";
  ctx.font = "700 24px system-ui, sans-serif";
  ctx.fillText(time, W - 18, 16);

  // HP segmented gauge + shield pips (bottom-left)
  const segW = 22;
  const segH = 12;
  const gap = 5;
  const hx = 18;
  ...
  for (let i = 0; i < MAX_HEALTH; i += 1) {
    const filled = i < game.health;
    ctx.fillStyle = filled
      ? (game.health <= 1 ? "#ef5a5f" : "#2cf28f")
      : "rgba(255, 255, 255, 0.12)";
    ctx.fillRect(hx + i * (segW + gap), hy, segW, segH);
  }
  for (let s = 0; s < game.shield; s += 1) {
    ctx.fillStyle = "#49b6ff";
    ctx.beginPath();
    ctx.arc(hx + MAX_HEALTH * (segW + gap) + 6 + s * 12, hy + segH / 2, 4, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.textAlign = "left";
  ctx.fillStyle = "rgba(233, 246, 241, 0.55)";
  ctx.font = "600 9px system-ui, sans-serif";
  ctx.fillText("HULL", hx, hy - 12);

  // Combo / fire gauge (bottom-right)
  const fw = 120;
  const fh = 10;
  const fx = W - 18 - fw;
  const fy = H - 29;
  const fill = clamp((game.combo - 1) / 8, 0, 1);
  ctx.fillStyle = "rgba(255, 255, 255, 0.12)";
  ctx.fillRect(fx, fy, fw, fh);
  ctx.fillStyle = game.scoreBoostTimer > 0 ? "#f7d64a" : "#e2b93b";
  ctx.fillRect(fx, fy, fw * fill, fh);
  ctx.textAlign = "left";
  ctx.fillStyle = "rgba(233, 246, 241, 0.55)";
  ctx.font = "600 9px system-ui, sans-serif";
  ...
  ctx.fillText(`×${game.combo}`, W - 18, fy - 14);
